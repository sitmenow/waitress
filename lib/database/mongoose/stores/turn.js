...
class TurnStore {
  ...
  async find(turnId) {
    ...
    if (!model) throw new TurnModelNotFound(turnId);
  ...
  async update(turn) {
    ...
    if (!model) throw new TurnModelNotFound(turn.id);
  ...
  _modelToObject(model) {
    ...
    try {
      ...
      throw new TurnEntityNotCreated(model.id, error.stack);
    }
  ...
  _objectToModel(turn) {
    ...
    try {
      ...
      throw new TurnModelNotCreated(turn.id, error.stack);
    }
